...
{
	//Cargar shaders
	std::string vertexShader{ loadTextFile("Shaders/vertexBumpMap.glsl") };
	std::string fragmentShader{ loadTextFile("Shaders/fragmentBumpMap.glsl") };
	//Crear programa
	...
	uniforms["valueX"] = glGetUniformLocation(shaders["transforms"], "cameraPos");
...
void Application::Setup()
{
	...
	SetupShaders();
	SetupPlane();
	textures["HeightMap"] = SetupTexture("Textures/RockNormal.jpg");
	textures["Diffuse"] = SetupTexture("Textures/RockTexture.jpg");
	//SetupGeometry();
...
void Application::Draw()
{
	...
	glUniform1f(uniforms["valueX"], valueX);
	glUniform1f(uniforms["valueY"], valueY);
	...
	glUniform2f(uniforms["cameraPos"], cameraPos.x,cameraPos.y);
